...
leafIndices = foldr (\(Dependency item prereqs) -> (flip (foldr insertWithNewIndex)) (item : prereqs)) Map.empty
    where insertWithNewIndex k m = if k `Map.member` m then m else Map.insert k (Map.size m) m
